-- Exercise set 3
-- Helped by: Adam Case, Colin Rodine, Website
--  * lists
...
firstHalf xs
    ...
    | odd(length xs) = take (length xs `div` 2 +1) xs
    | otherwise = ""
...
joinToLength k xs  =  filter (\x -> length x  == k)(joinToLengthCreate xs)

joinToLengthCreate  xs  = [x ++ y | x <- xs, y <- xs]
------------------------------------------------------------------------------
...
--(+|+) :: String -> [String]
--jointList :: String -> String -> String
--jointList str1 str2 = head str1 +|+ head str2
...
multiApp f xs x = f (multiApp1 xs x)

multiApp1 :: [a -> b] -> a -> [b]
multiApp1 xs x = map ($ x) xs
