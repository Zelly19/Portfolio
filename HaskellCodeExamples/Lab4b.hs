...
maxHelper x n = if n > x then n else x
...
slHelper k (n,v) = if k == k then (n+k,v+1) else (k,v)
...
--largestHelper x y = if x == maximum y || null y then x:y else y
--largestHelper :: Int -> [Int] -> [Int]
--largestHelper x [] = [x]
--largestHelper x y =
