...
instance Eq Name where
  (==) (Name y) (Name e)  = y == e || map toLower y == map toLower e
...
instance Eq a => Eq (List a) where
  ...
  (==) Empty _ = False 
  (==) _ Empty = False  
  (==) (LNode x Empty) (LNode a Empty) = a == x
  (==) (LNode x Empty) (LNode a (LNode z n)) = a == x  && Empty /= n 
  (==) (LNode x (LNode z n)) (LNode a Empty) = a == x  && Empty /= n 
  (==) (LNode x (LNode z n)) (LNode a (LNode p o)) = a == x  && o == n 

  --(==) (LNode x y) (LNode a b) = True
...
instance Ord Number where
    compare Infinite Infinite = EQ --In
    compare Infinite _ = GT --Infinite
    compare _ Infinite = LT --Infinite
    compare (Finite x) (Finite y) = compare x y


-- instance Ord Number where
--   (Finite x) (Finite y) = if x < y then x else y
--   Infinite _ = Infinite
...
instance Eq RationalNumber where
  RationalNumber q p == RationalNumber e r = q * r == p * e
...
simplify (RationalNumber q p) = RationalNumber (q `div` g) (p `div` g)
                                  where g = myGcd q p
...
instance Num RationalNumber where
  p + q = undefined
  (+) (RationalNumber p q) (RationalNumber d e) = simplify (RationalNumber ((p*e) + (q*d))  (q*e))
  (*) (RationalNumber p q) (RationalNumber d e) = simplify (RationalNumber (p*d) (q*e))
  abs (RationalNumber p q) = RationalNumber (abs p) (abs q)
  signum (RationalNumber p q) = RationalNumber (signum p * signum q) 1
